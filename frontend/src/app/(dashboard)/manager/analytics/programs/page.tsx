  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap gap-2">
        <Select value={programId} onValueChange={value => { setProgramId(value); setCohort("all") }}><SelectTrigger className="w-72"><SelectValue placeholder="Chọn ngành bắt buộc" /></SelectTrigger><SelectContent>{raw.programs.map(item => <SelectItem key={item.id} value={String(item.id)}>{item.name}</SelectItem>)}</SelectContent></Select>
        <Select value={semester} onValueChange={setSemester}><SelectTrigger className="w-52"><SelectValue placeholder="Học kỳ" /></SelectTrigger><SelectContent><SelectItem value="all">Tất cả học kỳ</SelectItem>{data.sortedSemesters.map(item => <SelectItem key={item.id} value={item.code}>{item.name}</SelectItem>)}</SelectContent></Select>
        <Select value={cohort} onValueChange={setCohort}><SelectTrigger className="w-44"><SelectValue placeholder="Khóa sinh viên" /></SelectTrigger><SelectContent><SelectItem value="all">Tất cả khóa</SelectItem>{data.cohorts.map(id => <SelectItem key={id} value={String(id)}>Khóa #{id}</SelectItem>)}</SelectContent></Select>
      </div>
