...
function studentIntervention(student: ApiDashboardSectionStudent) {
  ...
  if (grade < 4) {
    return { key: "meet_now", label: "Gặp ngay", reason: "Điểm tổng kết rất thấp", color: "#dc2626" }
  }
  if (grade < 5.5) {
    return { key: "academic_support", label: "Phụ đạo học thuật", reason: "Điểm dưới ngưỡng đạt", color: "#f97316" }
  }
  return { key: "monitor", label: "Theo dõi nhẹ", reason: "Đã đạt trong lớp học phần này", color: "#16a34a" }
}
...
export function SectionAnalyticsDetail({ sectionId }: { sectionId: number }) {
  ...
  async function askAi() {
    ...
    try {
      const answer = await api.askReportAgent({
        ...
        message: `Hãy phân tích lớp ${section?.section_code ?? sectionId} và đề xuất kế hoạch can thiệp học phần ưu tiên dựa trên aggregate, điểm tổng kết và danh sách sinh viên cần hỗ trợ.`,
        context: {
  ...
  const studentItems = React.useMemo(() => students?.items ?? [], [students?.items])
  ...
  const segments = React.useMemo(() => {
    ...
    return Array.from(map.values()).sort((a, b) => {
      const order = ["meet_now", "academic_support", "wait_for_grades", "monitor"]
      return order.indexOf(a.key) - order.indexOf(b.key)
    })
  }, [studentItems])
  const urgentStudents = React.useMemo(() => studentItems
    .map((student) => ({ student, segment: studentIntervention(student) }))
    .filter(({ segment }) => ["meet_now", "academic_support", "wait_for_grades"].includes(segment.key))
    .slice(0, 8), [studentItems])
  const sectionWarnings = React.useMemo(
    () => (students?.warnings ?? []).filter((warning) => !warning.toLocaleLowerCase("vi").includes("dropout")),
    [students?.warnings],
  )
  ...
  return <div className="space-y-6">
    ...
    {section ? <Card className={section.risk_level === "high" ? "border-red-500/30 bg-red-500/5" : "border-primary/20"} data-tour="page-section-detail-summary">
    ...
    {section ? <div className="grid gap-3 md:grid-cols-4">
    ...
    <div className="grid gap-4 xl:grid-cols-3">
      ...
      <Card><CardHeader><CardTitle className="text-base">Nhóm can thiệp</CardTitle><p className="text-xs text-muted-foreground">Chia sinh viên thành nhóm hành động từ điểm tổng kết của lớp học phần.</p></CardHeader><CardContent className="h-[320px]">{segments.length ? <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={segments} dataKey="value" nameKey="label" innerRadius={62} outerRadius={96} paddingAngle={3}>{segments.map((row) => <Cell key={row.key} fill={row.color} />)}</Pie><Tooltip content={<SegmentTooltip />} /><Legend /></PieChart></ResponsiveContainer> : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Chưa có dữ liệu sinh viên.</div>}</CardContent></Card>
      <Card className="xl:col-span-3" data-tour="page-section-detail-results"><CardHeader><CardTitle className="text-base">Kế hoạch ưu tiên theo điểm học phần</CardTitle><p className="text-xs text-muted-foreground">Danh sách ngắn để hỗ trợ sinh viên trong chính lớp học phần này.</p></CardHeader><CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{urgentStudents.length ? urgentStudents.map(({ student, segment }) => <Link key={student.student_id} href={`/manager/analytics/students/${student.student_id}`} className="block rounded-lg border p-3 transition hover:bg-muted/40"><div className="flex items-start justify-between gap-2"><div><div className="font-medium">{student.student_code}</div><div className="text-xs text-muted-foreground">{student.full_name}</div></div><Badge variant={segment.key === "meet_now" ? "destructive" : "outline"}>{segment.label}</Badge></div><div className="mt-2 text-xs text-muted-foreground">Điểm {formatNumber(student.final_grade)}</div><p className="mt-2 text-xs text-muted-foreground">{segment.reason}</p></Link>) : <div className="col-span-full py-12 text-center text-sm text-muted-foreground">Không có nhóm ưu tiên trong bộ lọc hiện tại.</div>}</CardContent></Card>
    </div>
    <Card><CardHeader className="flex-row items-center justify-between"><div><CardTitle className="text-base">Bằng chứng sinh viên</CardTitle><p className="mt-1 text-xs text-muted-foreground">Bảng này là worklist hành động theo điểm lớp học phần; click sinh viên để xem hồ sơ chi tiết.</p></div><Select value={risk} onValueChange={(value) => setRisk(value ?? "all")}><SelectTrigger className="w-40"><span>{risk === "all" ? "Mọi mức" : levelLabel(risk)}</span></SelectTrigger><SelectContent><SelectItem value="all">Mọi mức</SelectItem><SelectItem value="high">Ưu tiên</SelectItem><SelectItem value="watch">Theo dõi</SelectItem><SelectItem value="normal">Ổn định</SelectItem><SelectItem value="pending">Thiếu điểm</SelectItem></SelectContent></Select></CardHeader><CardContent>{students?.items.length ? <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-muted-foreground"><th className="p-3">Sinh viên</th><th className="p-3 text-right">Điểm</th><th className="p-3">Lý do chính</th><th className="p-3">Hành động gợi ý</th><th className="p-3">Trạng thái</th></tr></thead><tbody>{students.items.map((student) => { const segment = studentIntervention(student); return <tr key={student.student_id} className="border-b"><td className="p-3"><Link className="font-medium text-primary" href={`/manager/analytics/students/${student.student_id}`}>{student.student_code} - {student.full_name}</Link></td><td className="p-3 text-right">{formatNumber(student.final_grade)}</td><td className="p-3">{segment.reason}</td><td className="p-3"><Badge variant={segment.key === "meet_now" ? "destructive" : "outline"}>{segment.label}</Badge></td><td className="p-3"><Badge variant={student.risk_level === "high" ? "destructive" : "outline"}>{levelLabel(student.risk_level)}</Badge></td></tr> })}</tbody></table></div> : <div className="py-12 text-center text-sm text-muted-foreground">Không có sinh viên phù hợp.</div>}</CardContent></Card>
    {sectionWarnings.length ? <details className="rounded-md border bg-muted/20 p-3 text-sm text-muted-foreground"><summary className="cursor-pointer font-medium">Chất lượng dữ liệu</summary><ul className="mt-2 list-disc space-y-1 pl-5">{sectionWarnings.map((warning) => <li key={warning}>{warning}</li>)}</ul></details> : null}
  </div>
}
