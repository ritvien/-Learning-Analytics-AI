    ...
    async function loadData() {
      try {
        ...
        if (active) {
          ...
          const feGrades = gradesRes.map(mapApiEnrollmentToFeGrade)
  ...
  return (
    <div className="space-y-6">
      {isLoading ? (
        ...
        <>
          <div className="flex flex-col w-full bg-background text-foreground transition-colors duration-300 rounded-xl border border-border/50 shadow-sm p-4 overflow-x-auto">
            {/* ── Dashboard Header ── */}
            ...
            {/* ── Tree View Area ── */}
            <div className="flex-1 flex flex-col items-center mt-4 w-full min-w-max">
              {/* ── EPU Root Node ── */}
                        ...
                        {dept.nganhs.map((major) => {
                          ...
                          return (
                            <div
                              ...
                              className={`w-full px-1.5 py-1 rounded-md border flex flex-col gap-0.5 transition-all duration-150 cursor-pointer
                                ${status.colorClass}
