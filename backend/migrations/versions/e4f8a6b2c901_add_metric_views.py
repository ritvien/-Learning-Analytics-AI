"""Add metric views used by analytics and agent tools.

Revision ID: e4f8a6b2c901
Revises: d3a7c9f1b204
...
revision: str = "e4f8a6b2c901"
down_revision: str | None = "d3a7c9f1b204"
branch_labels: str | None = None
depends_on: str | None = None


def upgrade() -> None:
    """Create OLTP convenience views expected by dashboards and agent prompts."""
    op.execute(
        """
    ...
    op.execute(
        ...
        JOIN students st ON e.student_id = st.id AND st.program_id = pc.program_id
    ...
    op.execute(
        ...
        SELECT
            ...
            ROUND(SUM(ps.gpa_avg * ps.total_students) / NULLIF(SUM(ps.total_students), 0), 2) AS gpa_avg,
            ROUND(SUM(ps.fail_rate_avg * ps.total_students) / NULLIF(SUM(ps.total_students), 0), 4) AS fail_rate_avg
        FROM departments d
...
def downgrade() -> None:
    """Drop metric views in dependency order."""
    op.execute("DROP VIEW IF EXISTS vw_department_stats")
    op.execute("DROP VIEW IF EXISTS vw_program_stats")
    op.execute("DROP VIEW IF EXISTS vw_course_stats")
    op.execute("DROP VIEW IF EXISTS vw_section_stats")
